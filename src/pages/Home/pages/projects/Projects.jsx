...
export default function Projects(){
    ...
    return (
        <div className={`w-full scrollbar-hidden gap-y-[0.5rem] grid [&>div]:w-full [&>div]:flex [&>div]:flex-col [&>div]:gap-y-[0.5rem] [&>div>div]:bg-componentsColor [&>div>div]:p-[1rem] [&>div>div>span]:flex [&>div>div>span]:flex-row [&>div>div>span]:gap-[0.75rem] [&>div>div>span]:items-center [&>div>div>span>p]:font-bold [&>div>div>span>p]:text-[1.35rem] [&>div>div>span>p]:text-nowrap ${breakpoint >= 992 ? "h-full grid-cols-[2fr_1fr]" : "h-fit grid-cols-1"} ${breakpoint >= 768 ? "pt-[1rem] gap-y-[1rem] [&>div]:overflow-scroll [&>div]:h-full [&>div]:gap-y-[1rem] [&>div>div]:mx-[1rem] [&>div>div]:border [&>div>div]:border-borderColor [&>div]:gap-[1rem]" : "[&>div]:h-fit [&>div>div]:shadow-[0px_0px_10px_var(--color-componentsShadow)]"}`}>
            <div>
                <div className={breakpoint >= 768 ? "w-[calc(100%-2rem)] rounded-xl" : "w-full"}>
